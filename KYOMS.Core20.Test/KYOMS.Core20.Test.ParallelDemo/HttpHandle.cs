...
namespace KYOMS.Core20.Test.ParallelDemo
{
    public class HttpHandle : IDisposable
    {
        ...
        /// <returns></returns>
        private Dictionary<string, string> InitData(string logisticsInterface)
        {
            try
            {
                var data = new Dictionary<string, string>();

                data.Add("data_digest", BaseInfo.CreateDataDigest(logisticsInterface)); //创建签名
        ...
        public async Task<string> PostAsync(Dictionary<string, string> contentDictionary)
        {
            try
            {
                var content = new FormUrlEncodedContent(contentDictionary);
                var response = _httpClient.PostAsync(_postUrl, content);
